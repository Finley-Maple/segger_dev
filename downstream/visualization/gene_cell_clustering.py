...
def clustering_gene_cell_umap(attention_matrix, gene_names, cell_ids, 
    ...
    for n_neighbors in n_neighbors_list:
        for n_components in n_components_list:
            ...
            gene_results = _cluster_genes(attention_matrix, n_neighbors, 
...
def _cluster_cells(attention_matrix, cell_ids, n_neighbors, n_components, min_dist, 
    ...
    if visualization:
        create_cell_clustering_visualization(attention_matrix_T, coordinates, cluster_labels, 
                                            linkage_matrix, cell_ids, n_components, 
                                            figures_dir, f'cell_clustering_{param_suffix}_{metric}')
    
    return results

def _cluster_genes(attention_matrix, n_neighbors, n_components, min_dist, 
                  n_clusters, random_state, metric):
    ...
    if attention_matrix.shape[0] > 50:
        # Perform PCA preprocessing
        sc.pp.pca(adata_genes, n_comps=min(50, attention_matrix.shape[0]), random_state=random_state)
    
    # Perform UMAP
    sc.pp.neighbors(adata_genes, n_neighbors=min(n_neighbors, attention_matrix.shape[1]-1), 
                   random_state=random_state, use_rep='X_pca', metric=metric)
...
def create_cell_clustering_visualization(attention_matrix_T, coordinates, cluster_labels, 
                                        linkage_matrix, cell_ids):
